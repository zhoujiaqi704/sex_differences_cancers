...
dmpTable_COAD_permutation1000=list(permuted_pval_female=permuted_pval_female, permuted_beta_female=permuted_beta_female, 
		permuted_pval_male=permuted_pval_male, permuted_beta_male=permuted_beta_male)

save(dmpTable_COAD_permutation1000, file="./TCGA_COAD/dmpTable_COAD_permutation1000.rda")

#calculate the bonf
permuted_bonf_female=as.data.frame(matrix(NA,nrow=nrow(dmpTable_COAD_permutation1000$permuted_pval_female), ncol=1000),row.names=rownames(dmpTable_COAD_permutation1000$permuted_pval_female))
permuted_bonf_male=as.data.frame(matrix(NA,nrow=nrow(dmpTable_COAD_permutation1000$permuted_pval_female), ncol=1000),row.names=rownames(dmpTable_COAD_permutation1000$permuted_pval_female))

for(i in 1: 1000) {
	permuted_bonf_female[,i] <- p.adjust(dmpTable_COAD_permutation1000$permuted_pval_female[,i],"bonf")
		permuted_bonf_male[,i] <- p.adjust(dmpTable_COAD_permutation1000$permuted_pval_male[,i],"bonf")
}
