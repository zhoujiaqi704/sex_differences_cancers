...
PD.Perm = LIHC.pd[which(LIHC.pd$sex=="Male"),]
Methyl.Perm = LIHC.Combat_412[,match(PD.Perm$Sample_Name, colnames(LIHC.Combat_412))]


control <- PD.Perm$Sample_Name[PD.Perm$Sample_Group=="NAT"]
case <- PD.Perm$Sample_Name[PD.Perm$Sample_Group=="LIHC"]

n <- 20; m <- 115 #match the female sample size (NAT = 20; case = 115)


# Set up parallel backend
num_cores <- 8 # Adjust the number of cores according to your system
...
for (i in 1:1000) {
	...
	dmpTable.pval2[,i] = eb$p.value[,2]
	dmpTable.beta2[,i] = fit$coef[,2]
	#dmpTable.ses[,i] = ses[,2]
	#dmpTable.bonf[,i] = p.adjust(dmpTable.pval[,i],method="bonf")
...
dmpTable_LIHC_male.sampling1000=list(Pval = dmpTable.pval, meanDiff = dmpTable.beta, Bonf = dmpTable.bonf )

save(dmpTable_LIHC_male.sampling1000,file="./TCGA_LIHC/dmpTable_LIHC_male.sampling1000.rda")



#summarizing
PermP.LIHC.male.sampling1000_numBonf=as.data.frame(matrix(NA,nrow=1000,ncol=1)); colnames(PermP.LIHC.male.sampling1000_numBonf)="num.Bonf"
for(i in 1 : ncol(dmpTable.bonf)) {
	PermP.LIHC.male.sampling1000_numBonf[i,1] <- sum(as.numeric(dmpTable.bonf[,i]<0.05))
}

save(PermP.LIHC.male.sampling1000_numBonf,file="./TCGA_LIHC/PermP.LIHC.male.sampling1000_numBonf.rda")


###plot
library("ggplot2")
a <- ggplot(PermP.LIHC.male.sampling1000_numBonf, aes(x = num.Bonf))+ geom_histogram(color = "black", fill = "gray",bins=20) +
...
####differences female-male distribution
PermP.LIHC.male.sampling1000_numBonf$diff <- 734-PermP.LIHC.male.sampling1000_numBonf[,1]

library("ggplot2")
a <- ggplot(PermP.LIHC.male.sampling1000_numBonf, aes(x = diff))+ geom_histogram(color = "black", fill = "gray",bins=20) +
  geom_vline(aes(xintercept = 0),
             linetype = "dashed", linewidth = 1,color="purple")+
             labs(x="# mDMPs (bonf < 0.05): female vs. male",y="frequency",title="sex differences (subset LIHC males)")+
             theme_Publication()+
             theme(plot.title = element_text(hjust = 0.5,size=12),text=element_text(size=12))
ggsave("./TCGA_LIHC/LIHC_subsampling1000.males.numBonf.sexDiff.pdf")
...
#for female pval
PD.Perm = LIHC.pd[which(LIHC.pd$sex=="Female"),]
Methyl.Perm = LIHC.Combat_412[,match(PD.Perm$Sample_Name, colnames(LIHC.Combat_412))]
...
#calculate the bonf
LIHC.Boot1000_numBonf=as.data.frame(matrix(NA,nrow=1000,ncol=2)); colnames(LIHC.Boot1000_numBonf)=c("male","female")

for(i in 1: 1000) {
	mdmpTable.bonf[,i] <- p.adjust(mdmpTable.pval[,i],"bonf")
	fdmpTable.bonf[,i] <- p.adjust(fdmpTable.pval[,i],"bonf")
	LIHC.Boot1000_numBonf[i,1] <- sum(as.numeric(mdmpTable.bonf[,i]<0.05))
	LIHC.Boot1000_numBonf[i,2] <- sum(as.numeric(fdmpTable.bonf[,i]<0.05))
}

BootdmpTable.pval <- list(Female=fdmpTable.pval, Male=mdmpTable.pval)

save(BootdmpTable.pval,file="./TCGA_LIHC/fmdmpTable_LIHC_bootstrap1000.rda")
save(LIHC.Boot1000_numBonf,file="./TCGA_LIHC/LIHC.Boot1000_numBonf.rda")


#plot
dat = melt(LIHC.Boot1000_numBonf)
colnames(dat)[1]="sex"
	...
	p2 <- ggplot(dat, aes(x= value,fill=sex))+ 
		...
		theme_Publication()+
		geom_vline(aes(xintercept = mean(LIHC.Boot1000_numBonf$female, na.rm=T)), linetype = "dashed", linewidth = 1,color="#AF382F")+
  		geom_vline(aes(xintercept=mean(LIHC.Boot1000_numBonf$male, na.rm=T)),  color="#0D71A8", linetype="dashed", linewidth=1)+
		labs(x="# f(m)DMPs",y="Bootstrapped distribution",title="LIHC")+
		theme(legend.position=c("none"))
		#theme(legend.position=c(0.8,0.8),legend.title = element_blank())
ggsave("./TCGA_CrossTumor/DNAm_plot/Boostrap_fmdmp_numBonf_distribution_LIHC.pdf",width= 4,height=2.5)
