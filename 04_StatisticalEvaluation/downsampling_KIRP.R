...
#data load
load("./TCGA_KIRP/KIRP.pd.rda")
load("./TCGA_KIRP/KIRP.Combat_305.rda")

PD.Perm = KIRP.pd[which(KIRP.pd$sex=="Male"),]
Methyl.Perm = KIRP.Combat_305[,match(PD.Perm$Sample_Name, colnames(KIRP.Combat_305))]


P <- 1000 # Number of bootstrap samples 

control <- PD.Perm$Sample_Name[PD.Perm$Sample_Group=="NAT"]
case <- PD.Perm$Sample_Name[PD.Perm$Sample_Group=="KIRP"]

n <- 13; m <- 61 #match the female sample size (NAT = 13; case = 94)
...
dmpTable_KIRP_male.sampling1000=list(Pval = dmpTable.pval, meanDiff = dmpTable.beta, Bonf = dmpTable.bonf )

save(dmpTable_KIRP_male.sampling1000,file="./TCGA_KIRP/dmpTable_KIRP_male.sampling1000.rda")


#done
#summarizing
PermP.KIRP.male.sampling1000_numBonf=as.data.frame(matrix(NA,nrow=1000,ncol=1)); colnames(PermP.KIRP.male.sampling1000_numBonf)="num.Bonf"
for(i in 1 : ncol(dmpTable_KIRP_male.sampling1000$Bonf)) {
	PermP.KIRP.male.sampling1000_numBonf[i,1] <- sum(as.numeric(dmpTable_KIRP_male.sampling1000$Bonf[,i]<0.05))
}

save(PermP.KIRP.male.sampling1000_numBonf,file="./TCGA_KIRP/PermP.KIRP.male.sampling1000_numBonf.rda")


###plot
library("ggplot2")
a <- ggplot(PermP.KIRP.male.sampling1000_numBonf, aes(x = num.Bonf))+ geom_histogram(color = "black", fill = "gray",bins=20) +
  geom_vline(aes(xintercept = 8091),
             linetype = "dashed", size = 1,color="red")+
  geom_vline(aes(xintercept=mean(num.Bonf, na.rm=T)),   
             color="blue", linetype="dashed", linewidth=1)+
             labs(x="# mDMPs (bonf < 0.05)",y="frequency",title="1000 times subsampling (KIRP males)")+
             theme_Publication()+
             theme(plot.title = element_text(hjust = 0.5,size=12),text=element_text(size=12))
ggsave("./TCGA_KIRP/KIRP_subsampling1000.males.numBonf.pdf")
...
#calculate the bonf
KIRP.Boot1000_numBonf=as.data.frame(matrix(NA,nrow=1000,ncol=2)); colnames(KIRP.Boot1000_numBonf)=c("male","female")

for(i in 1: 1000) {
	mdmpTable.bonf[,i] <- p.adjust(BootdmpTable.pval$Male[,i],"bonf")
	fdmpTable.bonf[,i] <- p.adjust(BootdmpTable.pval$Female[,i],"bonf")
	KIRP.Boot1000_numBonf[i,1] <- sum(as.numeric(mdmpTable.bonf[,i]<0.05))
	KIRP.Boot1000_numBonf[i,2] <- sum(as.numeric(fdmpTable.bonf[,i]<0.05))
}


save(BootdmpTable.pval,file="./TCGA_KIRP/fmdmpTable_LUSC_bootstrap1000.rda")
save(KIRP.Boot1000_numBonf,file="./TCGA_KIRP/KIRP.Boot1000_numBonf.rda")
...
dat = melt(KIRP.Boot1000_numBonf)
colnames(dat)[1]="sex"
	...
	p2 <- ggplot(dat, aes(x= value,fill=sex))+ 
		...
		theme_Publication()+
		geom_vline(aes(xintercept = mean(KIRP.Boot1000_numBonf$female, na.rm=T)), linetype = "dashed", linewidth = 1,color="#AF382F")+
  		geom_vline(aes(xintercept=mean(KIRP.Boot1000_numBonf$male, na.rm=T)),  color="#0D71A8", linetype="dashed", linewidth=1)+
		labs(x="# f(m)DMPs",y="Bootstrapped distribution",title="KIRP")+
		theme(legend.position=c("none"))
		#theme(legend.position=c(0.8,0.8),legend.title = element_blank())
ggsave("./TCGA_CrossTumor/DNAm_plot/Boostrap_fmdmp_numBonf_distribution_KIRP.pdf",width= 4,height=2.5)
