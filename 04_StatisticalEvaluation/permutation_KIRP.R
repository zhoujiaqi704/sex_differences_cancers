...
for(i in 1:1000) {
  ...
  #shuttle sex and Dx labels
	...
	PD =separate(data = PD, col = permIdx, into = c("permGroup", "permSex"), sep = "_")
		...
		#fDMP
		design.F= model.matrix(~permGroup+SV1+SV2+SV3+SV4, data=PD.F)#desigh matrix for ref-based
    	...
    	permuted_beta_female[,i] =  fit.F$coef[,2]
			...
			#mDMP
			design.M= model.matrix(~permGroup+SV1+SV2+SV3+SV4, data=PD.M)#desigh matrix for ref-based
...
dmpTable_KIRP_permutation1000=list(permuted_pval_female=permuted_pval_female, permuted_beta_female=permuted_beta_female, 
		permuted_pval_male=permuted_pval_male, permuted_beta_male=permuted_beta_male)

save(dmpTable_KIRP_permutation1000, file="./TCGA_KIRP/dmpTable_KIRP_permutation1000.rda")

#calculate the bonf
permuted_bonf_female=as.data.frame(matrix(NA,nrow=nrow(dmpTable_KIRP_permutation1000$permuted_pval_female), ncol=1000),row.names=rownames(dmpTable_KIRP_permutation1000$permuted_pval_female))
permuted_bonf_male=as.data.frame(matrix(NA,nrow=nrow(dmpTable_KIRP_permutation1000$permuted_pval_female), ncol=1000),row.names=rownames(dmpTable_KIRP_permutation1000$permuted_pval_female))

for(i in 1: 1000) {
	permuted_bonf_female[,i] <- p.adjust(dmpTable_KIRP_permutation1000$permuted_pval_female[,i],"bonf")
		permuted_bonf_male[,i] <- p.adjust(dmpTable_KIRP_permutation1000$permuted_pval_male[,i],"bonf")
}
