...
#data load
load("./TCGA_COAD/COAD.pd.rda")
load("./TCGA_COAD/COAD.Combat_310.rda")

PD.Perm = COAD.pd[which(COAD.pd$sex=="Male"),]
Methyl.Perm = COAD.Combat_310[,match(PD.Perm$Sample_Name, colnames(COAD.Combat_310))]


P <- 1000 # Number of bootstrap samples 

control <- PD.Perm$Sample_Name[PD.Perm$Sample_Group=="NAT"]
case <- PD.Perm$Sample_Name[PD.Perm$Sample_Group=="COAD"]

n <- 17; m <- 125 #match the female sample size (NAT = 13; case = 94)
...
for (i in 1:1000) {
	...
	design= model.matrix(~Sample_Group+SV1+SV2+SV3+SV4+SV5, data=Perm.dataMethyl$pd)#desigh matrix for ref-based
...
dmpTable_COAD_male.sampling1000=list(Pval = dmpTable.pval, meanDiff = dmpTable.beta, Bonf = dmpTable.bonf )

save(dmpTable_COAD_male.sampling1000,file="./TCGA_COAD/dmpTable_COAD_male.sampling1000.rda")


#done
#summarizing
PermP.COAD.male.sampling1000_numBonf=as.data.frame(matrix(NA,nrow=1000,ncol=1)); colnames(PermP.COAD.male.sampling1000_numBonf)="num.Bonf"
for(i in 1 : ncol(dmpTable_COAD_male.sampling1000$Bonf)) {
	PermP.COAD.male.sampling1000_numBonf[i,1] <- sum(as.numeric(dmpTable_COAD_male.sampling1000$Bonf[,i]<0.05))
}

save(PermP.COAD.male.sampling1000_numBonf,file="./TCGA_COAD/PermP.COAD.male.sampling1000_numBonf.rda")


###plot
library("ggplot2")
a <- ggplot(PermP.COAD.male.sampling1000_numBonf, aes(x = num.Bonf))+ geom_histogram(color = "black", fill = "gray",bins=20) +
  geom_vline(aes(xintercept = 9025),
             linetype = "dashed", size = 1,color="red")+
  geom_vline(aes(xintercept=mean(num.Bonf, na.rm=T)),   
             color="blue", linetype="dashed", linewidth=1)+
             labs(x="# mDMPs (bonf < 0.05)",y="frequency",title="1000 times subsampling (COAD males)")+
             theme_Publication()+
             theme(plot.title = element_text(hjust = 0.5,size=12),text=element_text(size=12))
ggsave("./TCGA_COAD/COAD_subsampling1000.males.numBonf.pdf")
...
results <- foreach(i = c("Female", "Male"), .packages = c("limma","dplyr")) %dopar% {
  	...
  	case <- PD$Sample_Name[PD$Sample_Group == tumor & PD$sex == i]
		...
		#limma
		design= model.matrix(~Sample_Group+SV1+SV2+SV3+SV4+SV5, data=Boot.dataMethyl$pd)#desigh matrix for ref-based
  ...
  return(dmpTable.pval)
}

#stopCluster(cl)

BootdmpTable.pval <- list(Female = results[[1]], Male = results[[2]])



#calculate the bonf
COAD.Boot1000_numBonf=as.data.frame(matrix(NA,nrow=1000,ncol=2)); colnames(COAD.Boot1000_numBonf)=c("male","female")
...
for(i in 1: 1000) {
	...
	COAD.Boot1000_numBonf[i,1] <- sum(as.numeric(mdmpTable.bonf[,i]<0.05))
	COAD.Boot1000_numBonf[i,2] <- sum(as.numeric(fdmpTable.bonf[,i]<0.05))
}


save(BootdmpTable.pval,file="./TCGA_COAD/fmdmpTable_COAD_bootstrap1000.rda")
save(COAD.Boot1000_numBonf,file="./TCGA_COAD/COAD.Boot1000_numBonf.rda")
...
dat = melt(COAD.Boot1000_numBonf)
colnames(dat)[1]="sex"
	...
	p2 <- ggplot(dat, aes(x= value,fill=sex))+ 
		...
		theme_Publication()+
		geom_vline(aes(xintercept = mean(COAD.Boot1000_numBonf$female, na.rm=T)), linetype = "dashed", linewidth = 1,color="#AF382F")+
  		geom_vline(aes(xintercept=mean(COAD.Boot1000_numBonf$male, na.rm=T)),  color="#0D71A8", linetype="dashed", linewidth=1)+
		labs(x="# f(m)DMPs",y="Bootstrapped distribution",title="COAD")+
		theme(legend.position=c("none"))
		#theme(legend.position=c(0.8,0.8),legend.title = element_blank())
ggsave("./TCGA_CrossTumor/DNAm_plot/Boostrap_fmdmp_numBonf_distribution_COAD.pdf",width= 4,height=2.5)
