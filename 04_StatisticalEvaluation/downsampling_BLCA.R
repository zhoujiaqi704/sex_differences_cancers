...
load("./TCGA_BLCA/BLCA.Combat_422.rda")

PD.Perm = BLCA.pd[which(BLCA.pd$sex=="Male"),]
Methyl.Perm = BLCA.Combat_422[,match(PD.Perm$Sample_Name, colnames(BLCA.Combat_422))]


P <- 1000 # Number of bootstrap samples 

control <- PD.Perm$Sample_Name[PD.Perm$Sample_Group=="NAT"]
case <- PD.Perm$Sample_Name[PD.Perm$Sample_Group=="BLCA"]

n <- 10; m <- 102 #match the female sample size (NAT = 13; case = 94)
...
dmpTable_BLCA_male.sampling1000=list(Pval = dmpTable.pval, meanDiff = dmpTable.beta, Bonf = dmpTable.bonf )

save(dmpTable_BLCA_male.sampling1000,file="./TCGA_BLCA/dmpTable_BLCA_male.sampling1000.rda")


#done

#summarizing
PermP.BLCA.male.sampling1000_numBonf=as.data.frame(matrix(NA,nrow=1000,ncol=1)); colnames(PermP.BLCA.male.sampling1000_numBonf)="num.Bonf"
for(i in 1 : ncol(dmpTable.bonf)) {
	PermP.BLCA.male.sampling1000_numBonf[i,1] <- sum(as.numeric(dmpTable.bonf[,i]<0.05))
}

save(PermP.BLCA.male.sampling1000_numBonf,file="./TCGA_BLCA/PermP.BLCA.male.sampling1000_numBonf.rda")


###plot
library("ggplot2")
a <- ggplot(PermP.BLCA.male.sampling1000_numBonf, aes(x = num.Bonf))+ geom_histogram(color = "black", fill = "gray",bins=20) +
  geom_vline(aes(xintercept = 177), #fDMP at bonf<0.05
             linetype = "dashed", size = 1,color="red")+
  geom_vline(aes(xintercept=mean(num.Bonf, na.rm=T)),   
             color="blue", linetype="dashed", linewidth=1)+
             labs(x="# mDMPs (bonf < 0.05)",y="frequency",title="1000 times subsampling (BLCA males)")+
             theme_Publication()+
             theme(plot.title = element_text(hjust = 0.5,size=12),text=element_text(size=12))
ggsave("./TCGA_BLCA/BLCA_subsampling1000.males.numBonf.pdf")
...
#calculate the bonf
BLCA.Boot1000_numBonf=as.data.frame(matrix(NA,nrow=1000,ncol=2)); colnames(BLCA.Boot1000_numBonf)=c("male","female")

for(i in 1: 1000) {
	mdmpTable.bonf[,i] <- p.adjust(BootdmpTable.pval$Male[,i],"bonf")
	fdmpTable.bonf[,i] <- p.adjust(BootdmpTable.pval$Female[,i],"bonf")
	BLCA.Boot1000_numBonf[i,1] <- sum(as.numeric(mdmpTable.bonf[,i]<0.05))
	BLCA.Boot1000_numBonf[i,2] <- sum(as.numeric(fdmpTable.bonf[,i]<0.05))
}


save(BootdmpTable.pval,file="./TCGA_BLCA/fmdmpTable_LUSC_bootstrap1000.rda")
save(BLCA.Boot1000_numBonf,file="./TCGA_BLCA/BLCA.Boot1000_numBonf.rda")
...
dat = melt(BLCA.Boot1000_numBonf)
colnames(dat)[1]="sex"
	...
	p2 <- ggplot(dat, aes(x= value,fill=sex))+ 
		...
		theme_Publication()+
		geom_vline(aes(xintercept = mean(BLCA.Boot1000_numBonf$female, na.rm=T)), linetype = "dashed", linewidth = 1,color="#AF382F")+
  		geom_vline(aes(xintercept=mean(BLCA.Boot1000_numBonf$male, na.rm=T)),  color="#0D71A8", linetype="dashed", linewidth=1)+
		labs(x="# f(m)DMPs",y="Bootstrapped distribution",title="BLCA")+
		theme(legend.position=c("none"))
		#theme(legend.position=c(0.8,0.8),legend.title = element_blank())
ggsave("./TCGA_CrossTumor/DNAm_plot/Boostrap_fmdmp_numBonf_distribution_BLCA.pdf",width= 4,height=2.5)
