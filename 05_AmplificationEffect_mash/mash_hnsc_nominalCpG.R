...
data.hnsc = mash_set_data(BETA.HNSC, SE.HNSC)
bonf.HNSC <- Bonf.HNSC[which(Bonf.HNSC[,1] < 0.05 | Bonf.HNSC[,2] < 0.05),]

# random subset
#Pdat is the probes with p_value (all probes)
#Bonf is the union matrix of both < 0.05 probes
#sampling the number of Bonf probes from Pdat with p_thresh
Pdat = P.HNSC
pheno="HNSC"
random_subset <- function(seed=1, Bonf, p_thresh) {
	library(dplyr)	
    ...
    random <- numeric(0)
    # Coerce data to data frame if it's not already a data frame
    if (!is.data.frame(Bonf)) {
    data <- as.data.frame(Bonf)
  	}
...
g_list <- list()
for (i in 1:rep) {
    random <- random_subset(i, Bonf=bonf.HNSC, p_thresh=0.05)
    # mash
    #Pdat <- Pdat[!rownames(Pdat) %in% random,]
    results <- fit_mash(random, data=data.hnsc)
    mix <- results[[1]]
    g <- results[[2]]
    if (i==1) {
...
write.table(mixture, file=paste0(pheno,"_mixprop_100_all.txt"), sep="\t", row.names=FALSE) #mean mixture proportions values for 100 resampling fitting 66 hypotheses
...
write.table(pm_all, file=paste0(pheno,"_mash_pm_0.05.txt"), sep="\t", row.names=FALSE)
write.table(psd_all, file=paste0(pheno,"_mash_psd_0.05.txt"), sep="\t", row.names=FALSE)
write.table(lfsr_all, file=paste0(pheno,"_mash_lfsr_0.05.txt"), sep="\t", row.names=FALSE)
write.table(weights_all, file=paste0(pheno,"_mash_weights_0.05.txt"), sep="\t", row.names=FALSE)
